...
// zeng: TODO jni 不是很感兴趣 就不细看了

void initJNILrefs() {
    JNIFrame *frame = (JNIFrame *) getExecEnv()->last_frame;
    frame->next_ref = (Object **) frame;    // zeng: next_ref指示lrefs中下一个ref, 下一个ref的地址和frame的地址相同,表示lrefs的空间用完了
}

// zeng: 其实就是把frame结构体和ostack往后挪, 给lrefs腾出更大的空间
JNIFrame *expandJNILrefs(ExecEnv *ee, JNIFrame *frame, int incr) {
    ...
    memcpy(new_frame, frame, sizeof(JNIFrame)); // zeng: 复制frame的属性

    new_frame->ostack = (u4 *) (new_frame + 1);

    ee->last_frame = (Frame *) new_frame;   // zeng: 用新的这个栈帧替换掉旧的栈帧

    memset(frame, 0, incr * sizeof(Object *));  // zeng: 旧的栈帧置为0
...
// zeng: jni c方法里获取 local 引用
// zeng: scanThread 也会扫描这里有用到的对象进行标记
Object *addJNILref(Object *ref) {
    ExecEnv *ee = getExecEnv();
    JNIFrame *frame = (JNIFrame *) ee->last_frame;

    if (frame->next_ref == (Object **) frame) { // zeng: next_ref指示lrefs中下一个ref, 下一个ref的地址和frame的地址相同,表示lrefs的空间用完了
        JNIFrame *new_frame;
        if ((new_frame = expandJNILrefs(ee, frame, LIST_INCR)) == NULL) // zeng: 扩大lrefs空间, 由于frame结构体地址相应的往后挪了,这里返回新的frame结构体地址
            return NULL;
        frame = new_frame;
    }

    return *frame->next_ref++ = ref;    // zeng: 引用 放入 lrefs中, 更新next_ref
}

// zeng: jni c方法里删除 local 引用
...
jobject Jam_CallNonvirtualObjectMethodA(JNIEnv *env, jobject obj, jclass clazz,
    ...
    return (jobject) addJNILref(*(Object **) executeMethodList((Object *) obj,
                                                               (Class *) clazz, (MethodBlock *) methodID,
                                                               (u8 *) jargs));
}
